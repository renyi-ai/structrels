...
class LinearRelationOperator(RelationOperator):
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        if h is None:
            cache_key = self.mt.name + self.prompt_template + subject + str(self.h_layer)
            if self.use_cache and cache_key in SIMPLE_CACHE:
                h = SIMPLE_CACHE[cache_key]
            else:
                if do_prompt:
...
class JacobianIclMeanEstimator(LinearRelationEstimator):
    ...
    def __call__(self, relation: data.Relation) -> LinearRelationOperator:
        ...
        # -- collect first-order approximations for every sample ----------
        approxes: list[functional.Order1Approx] = []
        ...
        # -----------------------------------------------------------------
        # stack into one big tensor:  shape  (N, …)
        weight_stack = torch.stack([a.weight for a in approxes])  # (N, D_out, D_in)
        bias_stack   = torch.stack([a.bias   for a in approxes])  # (N, D_out)

        # *final* mean (what you already had)
        weight = weight_stack.mean(dim=0)
        bias   = bias_stack.mean(dim=0)

        # -----------------------------------------------------------------
        # NEW ► running means so you can inspect convergence
        #
        #   running_weight_mean[k]  =  mean of first k+1 samples  (k = 0…N−1)
        #
        n = weight_stack.shape[0]
        denom = torch.arange(1, n + 1, device=weight_stack.device, dtype=weight_stack.dtype)  # (N,)
        running_weight_mean = (weight_stack.cumsum(dim=0).transpose(0, -1) / denom).transpose(0, -1)
        running_bias_mean   =  bias_stack.cumsum(dim=0) / denom.view(-1, *([1] * (bias_stack.ndim - 1)))

        # optional helper: norms of the running means (one scalar per k)
        running_weight_norm = running_weight_mean.flatten(1).norm(dim=1)  # (N,)
        running_bias_norm   = running_bias_mean.flatten(1).norm(dim=1)    # (N,)
        ...
        operator = LinearRelationOperator(
            ...
            beta=self.beta,
            metadata={
                "Jh":  [a.metadata["Jh"].squeeze() for a in approxes],
                "|w|": [a.weight.norm().item() for a in approxes],
                "|b|": [a.bias.norm().item()  for a in approxes],
                "running_weight_mean": running_weight_mean,        # tensor (N, D_out, D_in)
                "running_bias_mean":   running_bias_mean,          # tensor (N, D_out)
                "running_weight_norm": running_weight_norm.tolist(),# list[float] length N
                "running_bias_norm":   running_bias_norm.tolist(), # list[float] length N
            },
        )
        return operator
...
@dataclass(frozen=True)
class JacobianIclMeanEstimator_old(LinearRelationEstimator):
    h_layer: Layer
    z_layer: Layer | None = None
    beta: float | None = None
    rank: int | None = None  # If None, don't do low rank approximation.
