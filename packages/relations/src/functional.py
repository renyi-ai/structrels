    ...
    *,
    ...
    with torch.inference_mode():
        ...
        for i in range(0, len(inputs.input_ids), batch_size):
            ...
            if save_embeds:
                layer_hidden_states = batch_outputs.hidden_states[layer_index]
                seq_lengths = inputs.attention_mask[i : i + batch_size].sum(dim=1)
                last_token_embeddings = layer_hidden_states[:, seq_lengths-1, :]
                embeds.append(last_token_embeddings)
    ...
    if save_embeds:
        return {"preds": predictions, "embeds": torch.cat(embeds, axis=0)}
    else:
        return predictions
