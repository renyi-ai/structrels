    ...
    for ri, relation in enumerate(dataset.relations):
        ...
        for trial in range(n_trials):
            ...
            try:
                ...
                if len(test_relation.samples) == 0:
                    continue
            ...
            for h_layer in h_layers:
                ...
                for rank in ranks:
                    ...
                    if efficacy is None:
                        logger.warning(
                            f"skipping rank result with no efficacy: {h_layer=} {rank=}"
                        )
                        continue
                ...
                matrix = operator.weight.detach().cpu().numpy()
                bias   = operator.bias.detach().cpu().numpy()
                extended = np.hstack((matrix, bias.T))

                fname = f"{relation.name.replace(' ', '_')}_layer={h_layer}.npy"

                results_dir_matrices = os.path.join(results_dir, 'matrices_hstacked')
                if not os.path.exists(results_dir_matrices):
                    os.makedirs(results_dir_matrices)

                path = os.path.join(results_dir_matrices, fname)
