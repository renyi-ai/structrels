...
class SweepTrainResults(DataClassJsonMixin):
    ...
    def best_rank(self, k: int = 1) -> SweepRankResults | None:
        """Return the best rank by efficacy, if available."""
        if not self.ranks:
            return None
    ...
    def summarize(self) -> None:
        ...
        if best_rank is None:
            logger.info(
                ...
                f"rank=N/A | efficacy=N/A | samples={[str(x) for x in self.samples]}"
            )
            return
        logger.info(
            "layer finished | "
...
class SweepLayerSummary(DataClassJsonMixin):
    ...
    rank: metrics.AggregateMetric | None
    efficacy: metrics.AggregateMetric | None
...
class SweepRelationResults(DataClassJsonMixin):
    ...
    def by_layer(
        ...
        ranks_by_layer = defaultdict(list)
        efficacies_by_layer = defaultdict(list)
        for trial in self.trials:
            for layer in trial.layers:
                ...
                best_rank = layer.result.best_rank(k=k)
                ...
                if best_rank is None:
                    continue
                ranks_by_layer[layer.layer].append(best_rank.rank)
                efficacies_by_layer[layer.layer].append(best_rank.efficacy[k - 1])

        betas_by_layer = {
            layer: metrics.AggregateMetric.aggregate([x[0] for x in results])
        ...
        rank_metrics_by_layer = {
            layer: metrics.AggregateMetric.aggregate(values)
            for layer, values in ranks_by_layer.items()
        }
        efficacy_metrics_by_layer = {
            layer: metrics.AggregateMetric.aggregate(values)
            for layer, values in efficacies_by_layer.items()
        }
        ...
        return {
            layer: SweepLayerSummary(
                ...
                rank=rank_metrics_by_layer.get(layer),
                efficacy=efficacy_metrics_by_layer.get(layer),
            )
    ...
    def best_by_efficacy(
        ...
        """Return the best layer and average beta for that layer."""
        results_by_layer = self.by_layer(k=k, beta=beta)
        results_with_efficacy = {
            layer: summary
            for layer, summary in results_by_layer.items()
            if summary.efficacy is not None
        }
        if not results_with_efficacy:
            logger.warning(
                f'no efficacy results for "{self.relation_name}", falling back to faithfulness'
            )
            return self.best_by_faithfulness(k=k, beta=beta)
        best_layer = max(
            results_with_efficacy,
            key=lambda x: results_with_efficacy[x].efficacy.mean,
        )
        return results_with_efficacy[best_layer]
    ...
    def summarize(self, k: int = 1) -> None:
        ...
        for la, summ in results_by_layer.items():
            rank_mean = f"{summ.rank.mean:.2f}" if summ.rank is not None else "N/A"
            efficacy_mean = (
                f"{summ.efficacy.mean:.2f}" if summ.efficacy is not None else "N/A"
            )
            logger.info(
                f"layer={la} | beta={summ.beta.mean:.2f} | recall@{k}={summ.recall.mean:.2f} | "
                f"rank={rank_mean} | efficacy={efficacy_mean}"
            )
